...
X.gspst <- sp.stable(x)
X.gspst
all.equal(fitted(lm(X.gsp ~ X.gspst - 1)), X.gsp, 
          check.attributes=FALSE) # span the same space
zapsmall(crossprod(X.gspst)) # orthonormal
round(cor(X.gspst), 3) # smaller correlations
kappa(cbind(1, X.gspst)) # better conditioned
```

The rows and columns of the matrix returned by `sp.stable()` are unnamed, because they no longer have the straightforward interpretations that they had in the 'raw' spline bases considered previously. As mentioned, however, `wald()` can reconstruct the parameters implied by the raw parametrization. To see how this works, let's return to the artificial data generated in Section 2 of the vignette:
```{r}
set.seed(12345)
x <- runif(200, 0, 10)
x <- sort(x)
...
```
The first line of output in each case shows the omnibus $F$-test for the 6 regression spline coefficients, and the subsequent table gives identical estimates, standard errors, tests, and confidence limits for the individual interpretable coefficients.
