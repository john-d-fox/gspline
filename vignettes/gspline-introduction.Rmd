...
In this vignette, we introduce what we term _generalized regresson splines_ and the `gspline()` function in the **gspline** package, which implements them. The vignette focuses on how to use `gspline()` to generate simple regression splines, including splines that are equivalent to familiar B-splines and natural splines. Another vignette develops the much greater generality and flexibility supported by `gspline()`.
