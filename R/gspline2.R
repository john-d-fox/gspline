...
gspline2 <- function(x,
    ...
    Cmat <- function( knots, degree, smoothness, lin = NULL, intercept = 0, signif = 3) {
            ...
            for ( i in seq_along(knots) ) {
                ...
                if ( max(sm) > -1 ) {  # sm = -1 corresponds to discontinuity
                    ...
                    dmat <- Xf( k, knots, dm, D = sm, FALSE ) -   
                        Xf( k, knots, dm, D = sm, TRUE )
    ...
    Dmat <- function(knots, degree, periodic = FALSE, signif = 3) {
        ...
        for (i in seq_len(n_knots - 1) ) {
            k <- knots[i]
            dmat <- Xf(k, knots, dm, D = seq(0,dm), FALSE, periodic = periodic) -
                Xf(k, knots, dm, D = seq(0,dm), TRUE, periodic = periodic)
        ...
        if(periodic) {
            dmat <- Xf(0, knots, dm, D = seq(0,dm), FALSE, periodic = periodic) -
                Xf(k, knots, dm, D = seq(0,dm), TRUE, periodic = periodic)
        ...
        for ( i in seq_along(degree)) {
            ...
            if ( dm > di ) {
                dmat <- diag( (length(knots) + 1) * (dm +1)) [
                    (i - 1)*(dm + 1) + 1 + seq( di+1,dm), , drop = FALSE]
                rownames( dmat ) = paste( "I.", i,".",seq(di+1,dm),sep = '')
                cmat = rbind( cmat, dmat)
            }
        }
    ...
    # Build constraints
    constraint_mat <- Dmat(knots, degree, periodic = periodic)[Dmat_smoothness_indices, ,drop = FALSE] 
...
#    if(debug) svs <- svd(A,nu=0,nv=0)
    ...
    ffun <- if (!missing(x) && orthonormalize){
	    ...
	    function(x, D = NULL, limit = 1){
	    	ret <- fun(x, D, limit)
	    	if (!inwald()) ret <- ret %*% TT
	    	colnames(ret) <- nms
	    	attr(ret, "transformation") <- TT
	    	class(ret) <- c('gspline_matrix', class(ret))
#	    	cat("\ninwald = ", inwald(), "\n")  # to show whether called from wald()
	    	ret
	    }
    } else fun
