...
gspline <- function(x,
  ...
  Cmat <-
    function(knots,
      ...
      if (!is.null(intercept))
        cmat = rbind(cmat, "Intercept" =
                       Xf(intercept, knots, dm, D =
                            0))
  ...
  Xf <- function(x,
    ...
    if (periodic)
      rownames(ret) <-
      paste0(rownames(ret), '/', signif(period, signif)) # new style
  ...
  Dmat <- function(knots,
    ...
    for (i in seq_len(n_knots - 1)) {
      ...
      if (periodic)
        rownames(dmat) <- paste0(rownames(dmat), '/', signif(max(k), signif))
      cmat <- rbind(cmat,  dmat)
    }
  ...
  Pcon <- function(knots, degree, periodic) {
    ...
    for (i in seq_along(degree)) {
      di <- degree[i]
      if (dm > di) {
        dmat <- diag((length(knots) + 1) * (dm + 1)) [(i - 1) * (dm + 1) + 1 + seq(di +
                                                                                     1, dm), , drop = FALSE]
        rownames(dmat) = paste("I.", i, ".", seq(di + 1, dm), sep = '')
        cmat = rbind(cmat, dmat)
      }
  ...
  Dmat_smoothness_indices <-
    unlist(lapply(seq_along(smoothness), function(i)
  ...
  if (!is.null(intercept)) {
    dest <-
      fun(rep(intercept, max(degree) + 1),
          D = 0:max(degree),
          limit = -1)
...
print.gspline <-
  function(x,
    ...
    ret <-
      (Filter(Negate(is.function),  sapply(ls(environment(
        x
      )), get, environment(x))))
    ret <-
