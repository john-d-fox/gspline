...
gspline <- function(x,
	...
	Cmat <- function( knots, degree, smoothness, lin = NULL, intercept = 0, signif = 3) {
	  ...
	  if( ! is.null(lin)) cmat <- rbind(cmat,lin)
	  rk = qr(cmat)$rank
	...
	Dmat <- function(knots, degree, periodic = FALSE, signif = 3) {
	  ...
	  for (i in seq_len(n_knots - 1) ) {
	    ...
	    rownames( dmat ) <- paste0( "C",seq(0,dm),'|',signif(k, signif)) 
	  ...
	  if(periodic) {
	    dmat <- Xf(0, knots, dm, D = seq(0,dm), FALSE, periodic = periodic) -
	      Xf(k, knots, dm, D = seq(0,dm), TRUE, periodic = periodic)
	    rownames( dmat ) <- paste( "C", seq(0,dm),'|', signif(k, signif),'/',signif(max(k), signif))
	    cmat <- rbind(cmat, dmat)
	  } else {
	    dmat <- Xf(k, knots, dm, D = seq(0,dm) , FALSE ,periodic = periodic) -
	      Xf(k, knots, dm, D = seq(0,dm) , TRUE ,periodic = periodic)
	...
	# - Identify rows of Dmat that are used for constraints
	...
	ffun <- if (!missing(x) && (orthonormalize||rescale)){
	  if(rescale) {
	    ...
	    fun_scaled <- gspline(knots = (knots - a)*b, degree = degree, smoothness = smoothness,
