# 2019_09_25 GM
# - added scale argument: if TRUE, x is rescaled when producing 
#   a numerically stable basis
...
orth_basis <- function(X, tol = 1e-9) {
  # returns an orthonormal basis
  # using the SVD
  ud <- svd(X,nv = 0)
  ud$u[,ud$d > tol, drop = FALSE]
}
...
#' such as derivatives of various orders, discontinuities, etc.
#' 
...
#' has a range between -10 and 10 should avert numerical problems. We intend to
#' address conditioning issues in a subsequent version of \code{gspline}.
#' 
...
#' 
#' \code{sp} can be called with additional arguments beyond \code{x} to geneate
#' portions of linear hypothesis matrices that can be used to test or estimate
#' various values or derivatives of the spline:
#' 
#' \describe{
#' 
...
#' 2))}, or more briefly: \code{sp <- gspline(c(5, 10), 3, 2)} since the
...
#' can be created with \code{sp <- gspline(c(0, 5, 10, 20), c(1, 3, 3, 3, 1),
#' c(2, 2, 2, 2))}.
#' 
#' A step function with steps at 0, 1 and 2 would have the form: \code{sp <-
#' gspline(c(0, 1, 2), 0, -1)}.
#' 
#' @param x a vector of values that provide an estimate of the range of 
#' values over which the closure returned by \code{gspline} will be evaluated.
#' It is only used if \code{rescale} and/or \code{orthonormalize} are TRUE.
...
#' @param tolerance largest postive number indistinguishable from 0; i.e.,
#' numbers smaller than \code{tolerance} in magnitude are taken to be 0.
#' @param stable if TRUE both \code{rescale} and \code{orthonormalize} are
#' set to TRUE. Default: FALSE.
#' @param rescale if TRUE rescale the argument so it has a range between
#' -10 and 10 in order to improve the numerical properties of the spline
#' basis. The estimated coefficients are not directly interpretable but the 
#' \code{\link{wald}} function is designed to compute and display interpretable coefficients.
#' To see interpretable estimated coefficients use: \code{wald(model)}. Also, hypothesis
#' matrices for the \code{\link{wald}} function are expressed as linear coefficients of
#' the interpretable coefficients.  Default: \code{stable}
#' @param orthonormalize if TRUE, transform the spline basis into a equivalent
#' orthonormal basis, after rescaling if \code{rescale = TRUE}. 
#' The estimated coefficients are not directly interpretable but the 
#' \code{\link{wald}} function is designed to compute and display interpretable coefficients.
#' To see interpretable estimated coefficients use: \code{wald(model)}. Also, hypothesis
#' matrices for the \code{\link{wald}} function are expressed as linear coefficients of
#' the interpretable coefficients.  Default: \code{stable}.
...
#' polynomial spline.
#' @section Returned spline closure
#' The closure returned by \code{gspline} generates a spline matrix to 
#' @section Warning: The estimands generated by spline functions created by
#' \code{gspline} are designed so the coefficients are interpretable as changes
#' in values or derivatives at knots. The resulting matrix is not designed to
#' have optimal numerical properties. 
...
#' 
#' sp.natural <- gspline(c(0,5,10,15), c(1,3,3,3,1), c(1,2,2,1)) 
#' 
...
#	knots <- f(knots) # adjust knots
	...
	# 2019_10_01 GM: new ffun
	# 
	# Applies rescaling if rescale == TRUE
	# then orthonormalizes if orthonormalize == TRUE.
	# The orthonormalization is done 'directly' using
	# the SVD.
	# Note that the 'G' matrix is not needed since 
	# 'wald' needs to transform the whole model matrix
	#

	ffun <- if (!missing(x) && (orthonormalize||rescale)){
	  if(rescale) {
	    ran <- range(x)
	    a <- mean(ran)
	    b <- 20/(ran[2]-ran[1])   # rescaled x goes from -10 to 10
	    fun_scaled <- gspline3(knots = (knots - a)*b, degree = degree, smoothness = smoothness,
	                           intercept = (intercept-a)*b, orthonormalize = FALSE)
	  }
	  function(x, D = NULL, limit = 1){
	    if(inwald() || !is.null(D)) {
	      ret <- fun(x, D, limit)
	    } else {  
	      ret <- if(rescale) fun_scaled((x-a)*b) else fun(x)
	      colnames(ret) <- NULL
	      if(orthonormalize) ret <- orth_basis(ret) 
	    }
	    if(rescale) attr(ret,'rescaled') <- c(a = a, b = b)
	    if(orthonormalize) attr(ret,'orthonormalize') <- TRUE
	    class(ret) <- c('gspline_matrix', class(ret))
	    ret
	  }
	} else fun
